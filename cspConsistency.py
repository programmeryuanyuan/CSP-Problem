...
class Con_solver(Displayable):
    ...
    # Update domain according to "to_do"
    # to_do：variables and constraints related to variables
    def make_arc_consistent(self, orig_domains=None, to_do=None):
        ...
        if orig_domains is None:
            ...
            orig_domains = self.csp.variables
        if to_do is None:
            # Initilization
            # Set
    ...
    def solve_one(self, domains=None, to_do=None):
        ...
        if any(len(new_domains[var]) == 0 for var in new_domains):
            return False
        # Make sure availiable value for every variable is unique
...
# The algorithm is invoked through this class
class Search_with_AC_from_CSP(Search_problem,Displayable):
    ...
    def neighbors(self,node):
        ...
        if var:
            # Partitioning to domain1 domain2
